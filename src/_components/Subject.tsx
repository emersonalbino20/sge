...
export default function Subject() {
	...
	const cursoOptions = dataCurse?.data?.data?.map((c) => {
	...
	const { postMatchSubject, postMatchError, postMatchLevel } =
		usePostMatchCurseToSubject();
		...
		<>
					...
					<DialogHeader>
						...
						<DialogDescription>
							<p className="text-base text-gray-800">
								altere uma informação da disciplina e em seguida click em{' '}
			...
			<Dialog>
				...
				<DialogContent className="sm:max-w-[425px] bg-white">
					<DialogHeader>
						<DialogTitle>
							Vincular Disciplina {dataSubjectId?.data?.nome}
						</DialogTitle>
						<DialogDescription>
							Vincula a disciplina de{' '}
							<span className="text-blue-500 font-medium">
								{dataSubjectId?.data?.nome}
							</span>{' '}
							em multiplos cursos.
						</DialogDescription>
					</DialogHeader>
													...
													render={({ field }) => (
														<FormItem>
															<Textarea
																id="descricao"
																className={
																	formCreate.formState.errors.descricao?.message
																		? 'animate-shake animate-once animate-duration-150 animate-delay-100 w-full text-md border-2 border-red-300 text-red-500 focus:text-red-600 font-semibold focus:border-red-500 py-4 mb-2'
																		: 'w-full text-md border-2 border-gray-300 text-gray-600 focus:text-blue-600 focus:font-semibold focus:border-sky-500 py-4 mb-2'
																}
																placeholder="Dê uma descrição ao curso."
