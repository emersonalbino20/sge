...
export default function ExportBulletin() {
	...
	const { dataSubjectsCurse } = useGetIdSubjectsCurseQuery(fieldCursoId);
	console.log(dataSubjectsCurse?.data?.data?.length);

	const [buscar, setBuscar] = React.useState(null);
	const options = data?.data?.data?.map((aluno) => {
	...
	const media = somaNotas / dataSubjectsCurse?.data?.data?.length;
									...
									<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
										...
										<div className="text-center">
											...
											<div
												...
												title={dataSubjectsCurse?.data?.data?.map((sub) => {
													return sub.nome;
												})}
											>
												{dataSubjectsCurse?.data?.data?.length}
											</div>
										</div>
									</div>
								</CardContent>
									...
									<ul>
										{dataNotes?.data?.data?.map((subject) => (
											<TabsTrigger
												...
												className="border-none text-wrap "
											>
