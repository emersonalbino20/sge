...
const GlobalSearch = () => {
  ...
  const [fieldCurso, fieldClasse, fieldTurma, fieldTeacher] = watch([
    'cursos',
    'classes',
    'turmas',
    'teacher',
  ...
  //Turmas
  const { data: dataClassTeacher } = useQuery({
    queryKey: ['classProfessor', fieldTurma],
    queryFn: () =>
  ...
  const { data: dataStdHistory } = useGetStudentHistoryQuery(idStudent);
  const { data: dataStdGrades } = useGetStudentGradesQuery(idStudent);
  const { data: dataStdAtGrade } = useGetStudentAtGradeQuery(idStudent);
  ...
  const dataStudent = students?.pages.flatMap((page) => page.data) ?? [];
  const optionStudents = dataStudent?.map((student) => {
    return { value: student.id, label: student.nomeCompleto };
  });
  ...
  const { dataTeacherSubjects } = useGetIdTeacherSubjectsdQuery(idTeacher);
  const { data: dataTeacherClass } = useGetIdTeacherClassQuery(
    idTeacher,
    fieldClasse
  );
  const { dataTeacherGrades } = useGetIdTeacherGradesQuery(idTeacher);
  ...
  const dataTeacher = teachers?.pages.flatMap((page) => page.data) ?? [];
  const options = dataTeacher?.map((teacher) => {
    return { value: teacher.id, label: teacher.nomeCompleto };
  });
  const handleChange = (selectedOption) => {
    setIdTeacher(selectedOption.value);
  };
  ...
  return (
    <section className="m-0 w-screen h-screen  bg-gray-50">
      ...
      <div className="container mx-auto p-6 space-y-6">
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2 bg-">
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      <div>
                        <Select
                          ...
                          placeholder="Buscar por professores..."
                        />
                      </div>
                          ...
                          <FormItem>
                            <FormControl>
                              ...
                              >
                                ...
                                {dataTeacherGrades?.data?.data?.map((field) => {
                                  return (
                                    <option value={`${field.id}`}>
                                      {field?.nome} Classe {field?.curso?.nome}
                                    </option>
                                  );
                                })}
                              </select>
                      ...
                      <>
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                          ...
                          >
                            ...
                            <p className="text-sm text-gray-600 mt-1">
                              Consultar todas as classes em que o professor
                              leciona no ano lectivo actual
                            </p>
                          </button>
                        </div>
                        <div className="w-full flex flex-col space-y-4 mt-2">
                          ...
                          {teacherClass && (
                            ...
                            >
                              <div className="flex items-center gap-2 font-bold text-slate-600">
                                <Building2 className="h-5 w-5" />
                                Visualizar turmas designadas para o professor
                                numa classe
                              </div>
                          ...
                          {teacherGrade && (
                            ...
                            >
                              <div className="flex items-center gap-2 font-bold text-slate-600">
                                <GraduationCap className="h-5 w-5" />
                                Vizualizar todas as classes em que o professor
                                leciona no ano lectivo actual
                              </div>
                    ...
                    {fieldCurso && fieldClasse && fieldTurma ? (
                      <>
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                          <button
                            className="p-4 text-left border-none rounded-lg hover:bg-gray-50 shadow-sm shadow-slate-600 transition-colors"
                            onClick={() => setClassTeacher(!classTeacher)}
                          >
                            <div className="flex items-center gap-2 font-medium">
                              <GraduationCap className="h-5 w-5" />
                              Corpo Docente
                        ...
                        <div className="w-full flex flex-col space-y-4 mt-2">
                          {classTeacher && (
                            ...
                            >
                              ...
                              {dataClassTeacher?.data?.data?.map(
                                (value, index) => {
                                  return (
                                    <ul key={index}>
                                      <li className="flex items-center focus:text-rose-500 text-slate-600">
