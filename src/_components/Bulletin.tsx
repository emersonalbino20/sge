...
export default function Bulletin() {
	...
	const [fieldCursoId, fieldDisciplinaId, fieldTrimestreId, fieldTurmaId] =
		watch(['cursoId', 'disciplinaId', 'trimestreId', 'turmaId']);
	...
	const [
		...
		queries: [
			...
			{
				queryKey: ['professoresCursosBulletin', 50],
				queryFn: () => getTurmasProfessores(50),
			},
			{
				queryKey: ['professoresDisciplinasBulletin', 50],
				queryFn: () => getDisciplinasProfessores(50),
			},
			{
				queryKey: ['turmasClasseBulletin', fieldCursoId],
				queryFn: () => getTurmasClasse(fieldCursoId),
				enabled: !!fieldCursoId,
			},
		],
	});
	...
	React.useEffect(() => {
		buscarAlunos();
	}, [fieldTurmaId, fieldCursoId, fieldDisciplinaId]);
					...
					<div className="w-full text-center text-4xl text-red-600 md:text-2xl lg:text-2xl">
						<div>
							...
							<p className="text-red-500 italic font-semibold text-sm cursor-pointer">
								<Link to={'/AcademicYearPage'}>Selecionar agora</Link>
							</p>
						</div>
					</div>
				</div>
																...
																{item.estado === 'indefinido' ? (
																	<Dialog>
																		...
																		<DialogContent className="sm:max-w-[425px] bg-white">
																			<DialogHeader>
																				<DialogTitle>
																					Adicionar nota ao {item.nomeCompleto}
																				</DialogTitle>
																				<DialogDescription>
																					Essa secção tem como objectivo
																					atribuir uma nota trimestral ao aluno.
																				</DialogDescription>
																			</DialogHeader>
																			<Form {...form}>
																				<form
																	...
																	<Dialog>
																		...
																		<DialogContent className="sm:max-w-[425px] bg-white">
																			<DialogHeader>
																				<DialogTitle>
																					Actualizar Nota {item.nomeCompleto}
																				</DialogTitle>
																				<DialogDescription>
																					...
																					<span className="text-xs">
																						...
																						Tome atenção nos filtros
																						selecionados, trimestre, disciplina
																						e classe.
																					</span>
																				</DialogDescription>
																			</DialogHeader>
