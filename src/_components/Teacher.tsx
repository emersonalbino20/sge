...
export default function Teacher() {
	...
	const disciplinaOptions = dataSubjets?.data?.data?.map((c) => {
	...
	const handleSubmitUnConnect = async (
		data: z.infer<typeof TFormUnConnect>,
		e
	) => {
		...
		<>
			...
			<Dialog>
				...
				<DialogContent className="sm:max-w-[425px] bg-white">
					<DialogHeader>
						<DialogTitle>
							Desvincular Professor(a){' '}
							{dataTeacherSubjects?.data?.data?.nomeCompleto}
						</DialogTitle>
						<DialogDescription>
							Essa secção tem como objectivo desvicular relação já existente
							entre professores e algumas disciplinas especifícas.
						</DialogDescription>
					</DialogHeader>
					<Form {...formUnConnect}>
						<form onSubmit={formUnConnect.handleSubmit(handleSubmitUnConnect)}>
							...
							<DialogFooter>
								<Button
									type="submit"
									title="desvincular"
									className="bg-red-500 border-red-500 text-white hover:bg-red-500 hover:text-white w-12"
									onClick={() => {
								...
								{dataTeacherSubjects?.data?.data?.length > 0 && (
									<fieldset>
										...
										<div className="w-full flex flex-col justify-between px-2">
											<div>
												<p className="font-thin text-sm">
													{dataTeacherSubjects?.data?.data?.map((value) => {
														return (
															<ol type="A">
																<li>{value.nome}</li>
															</ol>
