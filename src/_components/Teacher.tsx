...
import { AlertCircleIcon, AlertTriangle, CheckCircleIcon, ChevronLeft, ChevronRight, Loader, SaveIcon, Search} from 'lucide-react'
...
import { useGetIdTeacherGradesdQuery, useGetIdTeacherSubjectsdQuery, useGetIdTeacherdQuery, useGetTeacherQuery, usePostClassToTeacher, usePostDisMatchSubjectTeacher, usePostMatchSubjectTeacher, usePostTeacher, usePutTeacher } from '@/_queries/UseTeacherQuery'
...
import { useGetIdSubjectQuery, useGetSubjectQuery } from '@/_queries/UseSubjectQuery'
...
//Get
  ...
  const { data, isError, isLoading } = useGetTeacherQuery();
  ...
  const disciplinaOptions = dataSubjets?.data?.data?.map((c)=>{return {value: c.id, label: c.nome}});
...
//Put
    ...
    React.useEffect(()=>{
      ...
      formUnConnect.setValue('idProfessor', dataTeacherId?.data?.id);
      ...
      <>
       <Dialog >
            ...
            <DialogDescription>
              ...
              altere uma informação do registro click em <span className='font-bold text-blue-600'>actualizar</span> quando terminar.
  ...
  <FormField
      ...
      <div className='flex flex-col space-y-3 mb-5'>
        ...
        <FormControl>
        <select {...field} id='cursos' onChange={(e)=>{field.onChange(parseInt(e.target.value))}} >
...
formClasse.formState.errors.disciplinaId?.message &&'animate-shake animate-once animate-duration-150 animate-delay-100 w-full text-lg border-2 border-red-300 text-red-600 focus:text-red-700 focus:font-semibold focus:border-red-500 py-2 focus:outline-none rounded-md bg-white'} onChange={(e)=>{field.onChange(parseInt(e.target.value))}}>
    ...
    />
        ...
        <DialogFooter>
          <Button className='responsive-button bg-blue-600 border-blue-600 text-white hover:bg-blue-500 font-semibold' type='submit' 
...
<DialogContent className="max-w-[260px] sm:w-[260px] md:w-[260px] lg:w-[260px] xl:w-[260x] bg-white">
                      <DialogHeader>
                        <DialogTitle>Vincular Professor(a) {dataTeacherSubjects?.data?.data?.nomeCompleto}</DialogTitle>
                        <DialogDescription>
      ...
      />
          ...
          <Dialog >
            ...
            <DialogContent className="sm:max-w-[425px] bg-white">
                  <DialogHeader>
                    <DialogTitle>Desvincular Professor(a) {dataTeacherSubjects?.data?.data?.nomeCompleto}</DialogTitle>
                    <DialogDescription>
      ...
      <Dialog >
          ...
          <DialogContent className="sm:max-w-[425px] bg-white">
          <DialogHeader>
            <DialogTitle>Informações sobre {dataTeacherSubjects?.data?.data?.nomeCompleto}</DialogTitle>
            <DialogDescription >
          ...
          <div className="grid gap-4 py-4 bg-white">
            <div className="flex flex-col w-full"><fieldset>
                ...
                <div>
                    ...
                    <p className='font-thin text-sm'>{dataTeacherId?.data?.nomeCompleto}</p>
                </div>
                <div>
                      <label >Data de Nasc. </label>		
                      <p className='font-thin text-sm'>{dataTeacherId?.data?.dataNascimento}</p>
                </div>
                </div>
                <fieldset>
                ...
                <div>
                    ...
                    <p className='font-thin text-sm'>{dataTeacherId?.data?.contacto?.telefone}</p>
                </div>
                <div>
                  {dataTeacherId?.data?.contacto?.email &&
                    (<> <Label >E-mail</Label>
                    <p className='font-thin text-sm text-wrap'>{dataTeacherId?.data?.contacto?.email}</p></>)
                  }
                </div>
                </div>
                </fieldset>
            ...
            {dataTeacherSubjects?.data?.data?.length > 0 && 
            <fieldset>
                <legend className='text-sm'>Disciplinas Curricular</legend>
                <div className="w-full flex flex-col justify-between px-2">
                <div>
                    <p className='font-thin text-sm'>{dataTeacherSubjects?.data?.data?.map((value)=>{return (
                    <ol type='A'><li>{value.nome}</li></ol>)})}</p>
                </div>
                </div>
                </fieldset>
                }
                {dataTeacherGrades?.data?.data?.length > 0 && 
                <fieldset>
              <legend className='text-sm'>Classes Ministradas</legend>
              <div className="w-full flex flex-col justify-between px-2">
      <div>
            {dataTeacherGrades?.data?.data?.map((e)=>{
              return (<>
              <Label >Curso: {e.curso.nome}, {"("+e.nome+" classe);"}
              </Label>
              <br/>
              </>
              )
  ...
  const dadosPaginados = dadosFiltrados?.slice(inicio, fim);
    ...
    <Header />
      ...
      { (postClassToLevel === 1) &&(
          <AlertSucesso message={postClassToError} />  
      )
      }
      { (postClassToLevel === 2) &&(
        <AlertErro message={postClassToError} /> )
      }

      { (postDisMatchSubjectLevel === 1) &&(
          <AlertSucesso message={postDisMatchSubjectError} />  
      )
      }
      { (postDisMatchSubjectLevel === 2) &&(
        <AlertErro message={postDisMatchSubjectError} /> )
      }
    ...
    <DialogContent className="sm:max-w-[425px] sm:w-[260px] md:w-[300px] lg:w-[360px] xl:w-[400px] p-4 overflow-y-auto bg-white">
      <DialogHeader>
        ...
        <DialogDescription>
          preencha o formulário e em seguida click em <span className='font-bold text-blue-600'>cadastrar</span> quando terminar.
        </DialogDescription>
      </DialogHeader>
      <Form {...form} >
     <form onSubmit={form.handleSubmit(handleSubmitCreate)} >
      ...
      <FormField
          ...
          render={({field})=>(
            ...
            className={errors?.contacto?.email?.message && 'animate-shake animate-once animate-duration-150 animate-delay-100 w-full text-md border-2 border-red-300 text-red-500 focus:text-red-600 font-semibold focus:border-red-500 py-4 mb-2'}/>
      ...
      <FormField
          ...
          render={({field})=>(
            <FormItem>
            <FormControl>
            <Input id='tel' {...registerWithMask('contacto.telefone',['999999999'], {required: true})} className={errors?.contacto?.telefone?.message && 'animate-shake animate-once animate-duration-150 animate-delay-100 w-full text-md border-2 border-red-300 text-red-500 focus:text-red-600 font-semibold focus:border-red-500 py-4 mb-2'}/>
            </FormControl>
          <FormMessage className='text-red-500 text-base'/>
          </FormItem>
        )}/>
          ...
          {isLoading &&
            ...
            {!isError && dadosPaginados?.map((turno) => (
              <tr key={turno.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="text-sm text-gray-500">{turno.nomeCompleto}</div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="text-sm text-gray-500">{turno.dataNascimento}</div>
                </td>
