...
const auxPostGradeToCurse = (data) => {
  return axios.post(`http://localhost:8000/api/classes/`, {
    nome: data.nome,
    ordem: data.ordem,
    cursoId: data.idCursos,
...
const auxPostMatchCurseToCurse = (data) => {
  return axios.post(
    `http://localhost:8000/api/cursos/${data.idCursos}/disciplinas`,
    {
      disciplinas: data.disciplinas,
    }
  );
...
/* Delete */
const auxPostDisMatchCurseToSubject = (data) => {
...
export const usePostGradeToCurse = () => {
  ...
  const { mutate, variables } = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['cursos'] });
      queryClient.invalidateQueries({ queryKey: ['classescurso'] });
      setResp(`(${count}) Operação realizada com sucesso!`);
      setLevel(1);
      console.log('certo');
    },
...
export const usePostDisMatchSubjectToCurse = () => {
  ...
  const queryClient = useQueryClient();
  const { mutate } = useMutation({
    mutationFn: auxPostDisMatchCurseToSubject,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['cursos'] });
      setResp(`(${count}) Operação realizada com sucesso!`);
      setLevel(1);
...
export const useGetIdSubjectsCurseQuery = (id, bool = false) => {
  const { data, isFetched } = useQuery({
    queryKey: ['disciplinascurso', id],
    queryFn: () =>
      axios.get(`http://localhost:8000/api/cursos/${id}/disciplinas`),
    enabled: !!id,
  });
  return { dataSubjectsCurse: data };
};
